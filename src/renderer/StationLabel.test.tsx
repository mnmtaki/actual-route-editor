...
describe('StationLabel bilingual label group',()=>{
  ...
  it('keeps noisy vertical offsets centered for the full bilingual block',()=>{
    const top={...station,labelOffsetX:-5,labelOffsetY:-100},bottom={...station,labelOffsetX:5,labelOffsetY:100}
    const topGroup=render(<svg><StationLabel station={top} settings={DEFAULT_SETTINGS} showForeign/></svg>).container.querySelector('.station-label-group')!,bottomGroup=render(<svg><StationLabel station={bottom} settings={DEFAULT_SETTINGS} showForeign/></svg>).container.querySelector('.station-label-group')!
    expect(topGroup).toHaveAttribute('data-label-horizontal-anchor','middle');expect(topGroup).toHaveAttribute('data-label-vertical-anchor','above');expect(topGroup).toHaveAttribute('data-label-anchor-x','100');expect(topGroup).toHaveAttribute('data-label-anchor-y','100');expect(topGroup.querySelector('text')).toHaveAttribute('text-anchor','middle')
    expect(bottomGroup).toHaveAttribute('data-label-horizontal-anchor','middle');expect(bottomGroup).toHaveAttribute('data-label-vertical-anchor','below');expect(bottomGroup).toHaveAttribute('data-label-anchor-x','100');expect(bottomGroup).toHaveAttribute('data-label-anchor-y','300');expect(bottomGroup.querySelector('text')).toHaveAttribute('text-anchor','middle')
  })
  ...
  it('lays out imported AARC labels as one anchored block, including multiline nameS',()=>{
    const aarc:Station={...station,source:{format:'aarc',pointId:57,nameP:[0,16.2],labelAnchorMode:'aarc-block',stationNameFontWeight:'bold'},labelOffsetX:0,labelOffsetY:16.2}
    aarc.source.sourceStationNameRowHeight=52.5;aarc.source.sourceStationSubNameRowHeight=35
    const {container}=render(<svg><StationLabel station={aarc} settings={{...DEFAULT_SETTINGS,stationLabelSize:45.5,stationLabelFontFamily:'sans-serif',stationLabelFontWeight:700,stationForeignLabelSize:31.5,stationForeignLabelFontFamily:'sans-serif',stationForeignLabelFontWeight:700,stationForeignLabelColor:'#888888',foreignLabelGap:0}} showForeign/></svg>)
    ...
    expect(block.getAttribute('transform')).toBe('translate(0 0)')
    ...
    expect(texts[1]).toHaveAttribute('fill','#888888')
    expect(texts.every(text=>text.getAttribute('dominant-baseline')==='middle')).toBe(true)
