...
describe('project and SVG export', () => {
  ...
  it('recognizes Build 11 defaults and upgrades only those defaults to the new transfer length',()=>{const legacy=JSON.parse(serializeProject(demoProject));for(const key of ['transferMinorAxis','transferEndPadding','transferDotGap'])delete legacy.settings[key];const restored=parseProjectJson(JSON.stringify(legacy));expect(restored.settings.transferMinorAxis).toBe(19.5);expect(restored.settings.transferEndPadding).toBe(5.15);expect(restored.settings.transferDotGap).toBe(5)})
