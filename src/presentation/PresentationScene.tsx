...
export const PresentationScene = memo(function PresentationScene({ project, sequence, time, width, height, svgRef }: { project: ActualRouteProject; sequence: PresentationSequence; time: number; width: number; height: number; svgRef?: Ref<SVGSVGElement> }) {
  ...
  const historicalIdentityProject = useMemo(() => projectWithLineColorsAt(projectWithLineParentsAt({ ...project, lines: project.lines.map(line => ({ ...line, name: getLineNameAt(line, state.historyDate).name })) }, state.historyDate), state.historyDate), [project, state.historyDate])
