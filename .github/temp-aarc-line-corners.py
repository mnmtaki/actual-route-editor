...
module = r'''import type { ActualRouteProject, Line, Segment } from '../data/model'
...
'''
Path('src/geometry/aarcLinePath.ts').write_text(module, encoding='utf-8')

# Route imported AARC transit lines through the source-faithful corner builder.
path = Path('src/geometry/path.ts')
text = path.read_text(encoding='utf-8')
import_anchor = "import { getStationAnchorForLine } from '../data/stationAnchor'\n"
if "getAarcImportedSegmentPathSpans" not in text:
    text = text.replace(import_anchor, import_anchor + "import { getAarcImportedSegmentPathSpans } from './aarcLinePath'\n")
old = "export function getSegmentPathSpans(project: ActualRouteProject, segment: Segment, resolvedLineId = segment.lineId): PathSpan[] {\n  const points = getSegmentPoints(project, segment, resolvedLineId)"
new = "export function getSegmentPathSpans(project: ActualRouteProject, segment: Segment, resolvedLineId = segment.lineId): PathSpan[] {\n  const aarcSpans = getAarcImportedSegmentPathSpans(project, segment, resolvedLineId)\n  if (aarcSpans !== undefined) return aarcSpans\n  const points = getSegmentPoints(project, segment, resolvedLineId)"
if old not in text:
    raise SystemExit('path getSegmentPathSpans anchor missing')
text = text.replace(old, new)
old = "  if (points.length < 2 || (segment.mode !== 'smooth' && segment.mode !== 'rounded')) return points\n  const result: Point[] = []"
new = "  const usesAarcGeometry = segment.source?.format === 'aarc'\n  if (points.length < 2 || (!usesAarcGeometry && segment.mode !== 'smooth' && segment.mode !== 'rounded')) return points\n  const result: Point[] = []"
if old not in text:
    raise SystemExit('path curve samples anchor missing')
text = text.replace(old, new, 1)
old = "  if (segment.mode !== 'smooth' && segment.mode !== 'rounded') {\n    const start = segment.fromStationId === stationId ? points[0] : points.at(-1)!"
new = "  if (segment.source?.format !== 'aarc' && segment.mode !== 'smooth' && segment.mode !== 'rounded') {\n    const start = segment.fromStationId === stationId ? points[0] : points.at(-1)!"
if old not in text:
    raise SystemExit('path sample near station anchor missing')
text = text.replace(old, new, 1)
path.write_text(text, encoding='utf-8')

# Preserve exact source station point ids on every imported segment so clustered
# Stations cannot leak one line occurrence's free flag into another line.
path = Path('src/import-export/aarc.ts')
text = path.read_text(encoding='utf-8')
old = "          const sourceFromPointId = previousStation.source?.pointId\n"
new = "          const sourceFromPointId = previousStationSourceIndex === undefined ? previousStation.source?.pointId : geometryPoints[previousStationSourceIndex]?.id\n"
if old not in text:
    raise SystemExit('aarc sourceFromPointId anchor missing')
text = text.replace(old, new, 1)
old = "          const segmentSource = { format: 'aarc' as const, lineId: sourceLineId, sourceLineId, ...(timeSlice?.id !== null && timeSlice?.id !== undefined ? { sourceTimeSliceId: timeSlice.id } : {}),"
new = "          const segmentSource = { format: 'aarc' as const, lineId: sourceLineId, sourceLineId, ...(sourceFromPointId !== undefined ? { pointIds: [sourceFromPointId, sourcePoint.id] } : {}), ...(timeSlice?.id !== null && timeSlice?.id !== undefined ? { sourceTimeSliceId: timeSlice.id } : {}),"
if old not in text:
    raise SystemExit('aarc segmentSource anchor missing')
text = text.replace(old, new, 1)
path.write_text(text, encoding='utf-8')

# Regression tests for free-station and ordinary station corners across AR Segment boundaries.
path = Path('src/import-export/aarcFreeSemantics.test.ts')
text = path.read_text(encoding='utf-8')
anchor = "import { parseProjectJson, serializeProject } from './projectJson'\n"
addition = "import { getSegmentPathSpans } from '../geometry/path'\n"
if addition not in text:
    if anchor not in text: raise SystemExit('free test import anchor missing')
...
if insert.strip() not in text:
    ...
    if pos < 0: raise SystemExit('free test ending missing')
