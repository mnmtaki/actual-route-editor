...
export function parseProjectJson(text: string): ActualRouteProject {
  ...
  const project: ActualRouteProject = {
    ...
    lines: parsed.lines.map((line, index) => { const { styleOverrides: _ignored, parentLineId: _parentLineId, ...rest }=line, styleOverrides=normalizeLineStyleOverrides(line.styleOverrides), parentLineId=typeof line.parentLineId === 'string' && line.parentLineId.trim() ? line.parentLineId.trim() : undefined, nameHistory=normalizeLineNameHistory(line), parentHistory=normalizeLineParentHistory(line); const normalized=({ ...rest, ...(parentLineId ? { parentLineId } : {}), ...(nameHistory ? { nameHistory } : {}), ...(parentHistory ? { parentHistory } : {}), ...(styleOverrides?{styleOverrides}:{}), ...(typeof line.lineStyleId === 'string' && line.lineStyleId ? { lineStyleId: line.lineStyleId } : {}), ...normalizedDateFields(line), stationSequence: Array.isArray(line.stationSequence) ? line.stationSequence : [], ...(Array.isArray(line.lineBadges) ? {lineBadges:line.lineBadges.flatMap(value => normalizeLineBadge(value))} : {}), lineOrder: Number.isFinite(line.lineOrder) ? line.lineOrder : index, visible: line.visible !== false, locked: line.locked === true }); if(nameHistory)syncLineNameFromHistory(normalized); if(parentHistory)syncLineParentFromHistory(normalized); return normalized }),
