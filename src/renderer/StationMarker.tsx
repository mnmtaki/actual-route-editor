...
export function StationMarker({ project, station, time, selected, hitRadius = 24, onPointerDown, onLabelPointerDown }: {
  ...
  const { stationSize, stationStyleId, transferGapRatio, transferPaddingRatio } = project.settings
  const style = getStationStyle(stationStyleId)
  const selectionColor = lines[0]?.color ?? '#596161'
  const marker = lines.length > 1
    ? style.renderTransfer({ station, lines, size: stationSize, rotation: getTransferMarkerRotation(project, station.id, time), gapRatio: transferGapRatio, paddingRatio: transferPaddingRatio })
  ...
  return <g>
    ...
    {project.settings.labelsVisible && !station.labelHidden && <text x={station.x + station.labelOffsetX} y={station.y + station.labelOffsetY} className="station-label" onPointerDown={onLabelPointerDown}>{station.name}</text>}
  </g>
}
