...
import { getBuiltInStationStyle, getBuiltInTransferStyle } from '../data/presetRegistry'
...
export function parseProjectJson(text: string): ActualRouteProject {
  ...
  const requestedDefaultTransferStyleId = typeof (parsed as Record<string, unknown>).defaultTransferStyleId === 'string' && (parsed as Record<string, unknown>).defaultTransferStyleId ? String((parsed as Record<string, unknown>).defaultTransferStyleId) : undefined
