...
export interface OrdinaryStationRenderProps { station: Station; size: number; style?: StationStyle; lineColor?: string; backgroundColor?: string }
...
export function renderStationArtwork({ station, style, lineColor = '#596161', backgroundColor = '#f3f0e9' }: { station: Station; style: StationStyle; lineColor?: string; backgroundColor?: string }) {
  const fill = style.fillEnabled ? colorFor(style.fillColorMode, style.fillColor, backgroundColor, '#ffffff') : 'none'
  ...
  const halo = style.haloEnabled && style.haloWidth > 0
    ? shapeNode(style, station.x, station.y, style.width + style.haloGap * 2, style.height + style.haloGap * 2, { fill: 'none', stroke: style.haloColor, strokeWidth: style.haloWidth, strokeOpacity: style.haloOpacity, 'data-station-shape': `${style.shape}-halo` })
    : null
  const transform = style.rotation ? `rotate(${style.rotation} ${station.x} ${station.y})` : undefined
  return <g className="station-artwork" data-station-artwork={station.id} data-station-style-id={style.id} transform={transform}>{halo}{shapeNode(style, station.x, station.y, style.width, style.height, bodyAttrs)}</g>
}

export function StationArtwork({ station, style, lineColor, backgroundColor }: { station: Station; style: StationStyle; lineColor?: string; backgroundColor?: string }) {
  return <>{renderStationArtwork({ station, style, lineColor, backgroundColor })}</>
}
...
const ordinary=({station,size,style,lineColor,backgroundColor}:OrdinaryStationRenderProps)=>style ? <StationArtwork station={station} style={style} lineColor={lineColor} backgroundColor={backgroundColor}/> : <circle cx={station.x} cy={station.y} r={size/2} fill="white" data-testid={`station-${station.id}`}/>
