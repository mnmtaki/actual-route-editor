...
export function MobileShell({ project, selection, activeLineId, drawing = false, onSelectLine, onSelectBasemapPath = () => undefined, onChange, onAddLine, onOpenPresentation, onAddText, onAddBasemapPath, onImportProject, onImportBackground, onRemoveBackground, onExportProject, onExportSvg, onExportImage, onShareProject, onShareSvg, onFitAll, onZoomSelection, onDeleteSelection, onAddLineBadge, onPhasePreview, onStartPhaseDrawing, onFinishDrawing, canUndo, canRedo, onUndo, onRedo, nativeFiles = false }: {
  project: ActualRouteProject; selection: Selection; activeLineId: string | null; drawing?: boolean; onSelectLine: (id: string) => void; onSelectBasemapPath?: (id: string) => void; onChange: (project: ActualRouteProject) => void; onAddLine: () => void
  onOpenPresentation: () => void; onAddText: () => void; onAddBasemapPath?: (category: BasemapPathCategory) => void; onImportProject: () => void; onImportBackground: () => void; onRemoveBackground?: () => boolean; onExportProject: () => void; onExportSvg: () => void; onExportImage: () => void; onShareProject: () => void; onShareSvg: () => void; onFitAll: () => void; onZoomSelection: () => void
  onDeleteSelection: () => void; onAddLineBadge: (lineId: string) => void; onPhasePreview: (path: OpeningPhasePath | null) => void; onStartPhaseDrawing: (phaseId: string, lineId: string, stationId: string | null) => void; onFinishDrawing?: () => void
  ...
  return <div className="mobile-shell" data-testid="mobile-shell">
    ...
    {active && <><button className="mobile-drawer-scrim" aria-label="关闭移动端面板" onClick={close}/><aside className="mobile-drawer" role="dialog" aria-modal="true" aria-label={activeLabel} onPointerDown={event => event.stopPropagation()}>
      ...
      <div className="mobile-drawer-body" onFocusCapture={event => requestAnimationFrame(() => event.target instanceof HTMLElement && event.target.scrollIntoView({ block: 'nearest' }))}>
        ...
        {active === 'elements' && <><section className="mobile-drawer-section"><p>在地图中心添加文字后，可直接拖动位置。</p><button className="primary" onClick={() => { onAddText(); close() }}>添加自由文本</button></section><BasemapPathPanel project={project} selectedId={selection?.type === 'basemapPath' ? selection.id : undefined} onSelect={id => { onSelectBasemapPath(id); close() }} onAdd={category => { onAddBasemapPath?.(category); close() }} /></>}
