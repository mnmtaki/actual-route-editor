...
import { getOrientationAnchorLine, isLineOperationalAt, isRelationOperationalAt } from '../timeline/active'
...
export function getTransferMarkerLayout(project: ActualRouteProject, stationId: string, time: string, visibleRelationIds?: string[], endPadding = 0): TransferMarkerLayout {
  ...
  const relations = getCompoundStationRelations(project, stationId)
    .filter(relation => visibleIdSet ? visibleIdSet.has(relation.id) : isRelationOperationalAt(relation, time))
    .filter(relation => {
      const line = project.lines.find(item => item.id === relation.lineId)
      return Boolean(line && line.visible && isLineOperationalAt(line, time))
    })
