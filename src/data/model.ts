...
export interface StationStyle {
  ...
  fillColor: string
  fillOpacity: number
  fillColorMode: StationStyleColorMode
  strokeEnabled: boolean
  strokeColor: string
  strokeWidth: number
  strokeOpacity: number
  strokeColorMode: StationStyleColorMode
  haloEnabled: boolean
  haloColor: string
  haloWidth: number
  haloGap: number
...
export interface LineNameHistoryEntry { id: string; effectiveAt: string | null; name: string }
export interface LineParentHistoryEntry { id: string; effectiveAt: string | null; parentLineId: string | null }
export interface LineColorHistoryEntry { id: string; effectiveAt: string | null; color: string }
export interface LineDisplayCodeHistoryEntry { id: string; effectiveAt: string | null; displayCode: string }
...
export interface NativeLineLabel { id: string; x: number; y: number; size: number; cornerRadius?: number; rotation: number; visible: boolean }
/** Legacy storage name kept for project JSON compatibility. User-facing concept is Line Label. */
export type LineBadge = NativeLineLabel
export type LineLabelSource = 'native' | 'aarc'
export interface Line { id: string; name: string; nameHistory?: LineNameHistoryEntry[]; parentHistory?: LineParentHistoryEntry[]; colorHistory?: LineColorHistoryEntry[]; displayCodeHistory?: LineDisplayCodeHistoryEntry[]; nameSub?: string; number?: string; code?: string; shortName?: string; displayCode?: string; color: string; parentLineId?: string; /** Visual-only/helper line. It is rendered and editable but excluded from passenger/operation statistics. */ isFake?: boolean; lineWidth?: number; styleOverrides?: LineStyleOverrides; lineStyleId?: string; stationSequence: string[]; lineBadges?: LineBadge[]; lineOrder: number; openedAt?: ISODate; closedAt?: ISODate; operationHistory?: OperationHistoryEntry[]; visible: boolean; locked: boolean; source?: SourceMetadata }
...
/** Presence of styleAfter means this boundary explicitly owns the following interval's line-style state. An empty object explicitly means "inherit the Line style". */
export interface StylePointStyleState { lineStyleId?: string | null }
export interface StructureNode { id: string; structureAfter: StructureType; waypointId?: string; progress?: number; styleAfter?: StylePointStyleState }
...
export interface ActualRouteProject { version: 1; name: string; projectName?: string; distanceScale?: DistanceScale; stations: Station[]; lines: Line[]; stationLineRelations: StationLineRelation[]; openingPhases: OpeningPhase[]; operationEvents?: OperationEvent[]; geometry: { segments: Segment[] }; mapElements?: MapElement[]; textTags?: AarcTextTag[]; aarc?: AarcSourceMetadata; lineLegend?: LineLegend; basemapPaths?: BasemapPath[]; roads?: Road[]; roadStyles?: RoadStyle[]; background: BackgroundImage | null; timeline: TimelineSettings; presentation: PresentationSettings; settings: ProjectSettings; styles?: LineStyle[]; stationStyles?: StationStyle[]; defaultStationStyleId?: string; transferStyles?: TransferStyle[]; defaultTransferStyleId?: string }
export type Selection = { type: 'station'; id: string } | { type: 'line'; id: string } | { type: 'lineLabel'; id: string; lineId: string; source: LineLabelSource } | { type: 'segment'; id: string; progress?: number } | { type: 'waypoint'; id: string; segmentId: string } | { type: 'structureNode'; id: string; segmentId: string } | { type: 'mapElement'; id: string } | { type: 'lineLegend'; id: string } | { type: 'basemapPath'; id: string } | { type: 'road'; id: string } | { type: 'roadPoint'; id: string; roadId: string } | { type: 'background' } | null

export const DEFAULT_STATION_LABEL_FONT_FAMILY = 'Inter, "Noto Sans SC", "Microsoft YaHei", sans-serif'
export const DEFAULT_SETTINGS: ProjectSettings = { lineWidth: 18, stationSize: 11, stationStyleId: 'default', transferMinorAxis: 19.5, transferEndPadding: 5.15, transferDotGap: 5, stationLabelSize: 14, stationLabelFontFamily: DEFAULT_STATION_LABEL_FONT_FAMILY, stationLabelFontWeight: 650, stationLabelColor: '#202526', stationForeignLabelSize: 10.08, stationForeignLabelFontFamily: DEFAULT_STATION_LABEL_FONT_FAMILY, stationForeignLabelFontWeight: 520, stationForeignLabelColor: '#202526', foreignLabelGap: 2.52, defaultLabelDirection: 'upper-right', defaultLabelDistance: 19.79898987322333, defaultStationLabelRotation: 0, transferHeightRatio: 1.0833333333333333, transferGapRatio: 0.1944, transferPaddingRatio: 0.25, labelsVisible: true, showForeignStationNames: true, gridVisible: true, exportBackground: true, worldUnitsPerKm: 100 }
