...
export function NetworkCanvas({ project, selection, selectedStationIds = [], onToggleStationSelection, drawing, roadDraft, phasePreview, calibration, onCalibrationPoint, onSelect, onCreatePoint, onConnectStation, onExtend, onFinishDrawing, onSegmentPoint, onPreview, onDragCommit, onEditBlocked, view, setView }: {
  ...
  const lineDrawingOverlay = (() => {
    ...
    const line = lineWithEffectiveColor(shown, rawLine), points = lineDraft.points
  ...
  return <svg id="network-canvas" ref={svgRef} className={`network-canvas ${drawing ? 'is-drawing' : ''}`} tabIndex={0} viewBox={`${view.x} ${view.y} ${view.width} ${view.height}`}
    ...
    <g data-layer="segments">{active.lines.flatMap(rawLine => {
      ...
      const line = lineWithEffectiveColor(shown, rawLine)
    ...
    <g data-layer="stations">{active.stations.map(station => <StationMarker key={station.id} part="marker" project={shown} station={station} time={shown.timeline.currentDate} selected={(selection?.type === 'station' && (selection.id === station.id || getCompoundStationCanonical(shown, selection.id)?.id === station.id)) || selectedStationIds.includes(station.id)} hitRadius={stationHitRadius}
    ...
    <g data-layer="station-labels">{active.stations.map(station => <StationMarker key={station.id} part="label" project={shown} station={station} time={shown.timeline.currentDate} selected={false} hitRadius={stationHitRadius} onPointerDown={() => {}} onLabelPointerDown={event => { if (drawing) return; if (startObjectDrag('draggingLabel', event, { x: station.labelOffsetX, y: station.labelOffsetY }, station.id)) onSelect({ type: 'station', id: station.id }) }} />)}</g>
    {lineDrawingOverlay}
