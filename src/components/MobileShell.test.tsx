...
describe('MobileShell', () => {
  ...
  it('keeps project, AARC, background import and image export in the mobile export drawer', () => {
    ...
    expect(screen.getByRole('button', { name: '导入拓扑' })).toBeTruthy()
  ...
  it('enters shared line multi-select mode on a long press and exposes batch actions', () => {
    ...
    fireEvent.click(row.querySelector('.line-row-main')!)
    expect(screen.queryByText('长按线路进入多选模式')).toBeNull()
    fireEvent.click(view.container.querySelector('[data-line-id="line-b"] .line-row-main')!)
    fireEvent.click(view.container.querySelector('[data-line-id="line-c"] .line-row-main')!)
    expect(onToggle).toHaveBeenNthCalledWith(1, 'line-b')
    expect(onToggle).toHaveBeenNthCalledWith(2, 'line-c')
