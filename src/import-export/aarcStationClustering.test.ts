...
describe('AARC station connectivity graph', () => {
  ...
  it('supports pointLinks and transitive helper bridges without materializing helper-only components', () => {
    const result = buildAarcStationComponents([
      point(10, 0, 0, 0, '主站'), point(11, 25, 0), point(12, 50, 0), point(13, 500, 0),
    ], new Map([[10, [1]], [12, [2]]]), [{ pts: [10, 11], type: 1 }])
  ...
  it('warns for invalid pointLinks without dropping valid station connections', () => {
    const result = buildAarcStationComponents([point(1, 0, 0), point(2, 100, 0)], new Map([[1, [1]], [2, [2]]]), [{ pts: [1, 999, 2], type: 1 }])
  ...
  it('merges the complete 常陵 interchange fixture through proximity, pointLinks and shared ids', () => {
    ...
    expect(relationNames('稻香楼')).toEqual(['1', '17', '4'])
    ...
    expect(summary.stationCount).toBe(436)
    ...
    expect(audit.metrics).toEqual({ sta1Total: 465, referencedSta1Count: 463, helperCount: 2, proximityEdgeCount: 29, pointLinksEdgeCount: 1, multiPointComponentCount: 25, interchangeStationCount: 126, helperOnlyComponentCount: 0, ambiguousNameCount: 0 })
  })
})
