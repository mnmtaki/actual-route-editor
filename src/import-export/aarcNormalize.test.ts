...
describe('AARC save normalization and source sizing', () => {
  it('applies upstream defaults without mutating the raw save and normalizes singleton registries', () => {
    ...
    expect(normalized.config.snapOctaClingPtPtDist).toBe(25)
    expect('freePtClusterMode' in normalized.config).toBe(false)
  ...
  it('resolves common-line width ratios, overrides and aggregate station metrics deterministically', () => {
    ...
    const lines = [{ id: 1, type: 0, width: 1, ptSize: 2 }, { id: 2, type: 0, width: 2, ptSize: 5 }]
    expect(aggregateAarcPointMetrics(10, lines, new Map([[10, [1, 2]]]), config)).toMatchObject({ ptSize: 5, ptSnapSize: 5, ptNameSnapSize: 5 })
  })

  it('uses min(width, 1) for terrain station render/snap size but excludes terrain from name metrics', () => {
    const config = normalizeAarcSave({ lines: [], points: [], cvsSize: [1, 1], config: {} }).config
    const lines = [
      { id: 1, type: 0, width: 0.5, ptNameSize: 2, ptNameSnapSize: 3 },
      { id: 2, type: 1, width: 4, ptSize: 9, ptSnapSize: 9, ptNameSize: 20, ptNameSnapSize: 20 },
    ]
    expect(aggregateAarcPointMetrics(10, lines, new Map([[10, [1, 2]]]), config)).toEqual({ ptSize: 1, ptNameSize: 2, ptSnapSize: 1, ptNameSnapSize: 3 })
  })
