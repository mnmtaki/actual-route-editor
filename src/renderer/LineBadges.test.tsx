...
describe('square LineBadge renderer',()=>{
  ...
  it('uses an explicit persisted corner radius and clamps it to half the label size',()=>{const project=projectWith('4',50);project.lines[0].lineBadges![0].cornerRadius=6;expect(frame(project).rect).toHaveAttribute('rx','6');project.lines[0].lineBadges![0].cornerRadius=99;expect(frame(project).rect).toHaveAttribute('rx','25')})
  it('keeps the previous proportional radius when an old badge has no cornerRadius',()=>{expect(frame(projectWith('4',50)).rect).toHaveAttribute('rx','11')})
