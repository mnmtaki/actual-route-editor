...
describe('Inspector station history editing', () => {
  ...
  it('offers adding a badge only from the selected Line inspector',()=>{const onAddLineBadge=vi.fn();render(<Inspector project={structuredClone(demoProject)} selection={{type:'line',id:'line-a'}} onChange={()=>{}} onDelete={()=>{}} onAddLineBadge={onAddLineBadge} onPhasePreview={()=>{}} onStartPhaseDrawing={()=>{}}/>);fireEvent.click(screen.getByRole('button',{name:'添加线路标签'}));expect(onAddLineBadge).toHaveBeenCalledWith('line-a')})
