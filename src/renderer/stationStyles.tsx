...
const presentation=({station,previousLines,lines,size,minorAxis,dotGap,endPadding,rotation,morphProgress,opacity,scale,centerX=centerOf(station).x,centerY=centerOf(station).y,minMajorAxis=0}:PresentationStationRenderProps)=>{const previousCount=previousLines.length;if(lines.length<2)return <g opacity={opacity} transform={`translate(${station.x} ${station.y}) scale(${scale}) translate(${-station.x} ${-station.y})`}>{ordinary({station,size})}</g>;const fromMetrics=getDefaultTransferMetrics(size,Math.max(2,previousCount),dotGap,endPadding,minorAxis,minMajorAxis),toMetrics=getDefaultTransferMetrics(size,lines.length,dotGap,endPadding,minorAxis,minMajorAxis),morph=morphProgress,width=lerp(previousCount<2?size:fromMetrics.width,toMetrics.width,morph),height=lerp(previousCount<2?size:fromMetrics.height,toMetrics.height,morph);return <g opacity={opacity} transform={`rotate(${rotation} ${centerX} ${centerY})`} data-presentation-station={station.id}>{previousCount<2&&<circle cx={station.x} cy={station.y} r={size/2} fill="white" opacity={1-morph}/>}<rect x={centerX-width/2} y={centerY-height/2} width={width} height={height} rx={height/2} {...TRANSFER_CONTAINER_STYLE} opacity={previousCount<2?morph:1}/>{lines.map((line,index)=>{const previousIndex=previousLines.findIndex(item=>item.id===line.id),fromX=previousIndex>=0&&previousCount>=2?dotX(centerX,fromMetrics,previousIndex):centerX,toX=dotX(centerX,toMetrics,index);return <circle key={line.id} cx={lerp(fromX,toX,morph)} cy={centerY} r={toMetrics.dotDiameter/2} fill={line.color} opacity={previousIndex>=0&&previousCount>=2?1:morph}/>})}</g>}
