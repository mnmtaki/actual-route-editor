...
export function Toolbar({ canUndo, canRedo, undo, redo, fitAll, zoomSelection, importProject, importTopology, exportProject, exportSvg, exportImage = () => undefined, shareProject: _shareProject = () => undefined, shareSvg: _shareSvg = () => undefined, importBackground, onNewLine, onAddText = () => undefined, onAddRoad = () => undefined, onAddBasemapPath = () => undefined, onAddLineLegend = () => undefined, onSelectLineLegend = () => undefined, lineLegend, basemapPaths, onSelectBasemapPath = () => undefined, onStyle, onSettings = () => undefined, drawing, onFinish, onPresentation, snapOptions = { node: false, neighbor: false, grid: false }, onToggleSnapOption = () => undefined, nativeFiles = false, projectName = '未命名工程', onProjectNameChange = () => undefined }: {
  ...
  return <header ref={rootRef} className="toolbar direct-toolbar">
    ...
    <div className="tool-group desktop-tools"><button onClick={fitAll}>适应全部</button><button onClick={zoomSelection}>缩放到选择</button><TopDropdown id="snap" label="吸附" openMenu={openMenu} setOpenMenu={setOpenMenu} className="snap-menu"><button role="menuitemcheckbox" aria-checked={snapOptions.neighbor} className={`snap-option ${snapOptions.neighbor ? 'is-enabled' : ''}`} onClick={() => onToggleSnapOption('neighbor')}><span>邻点延长线</span><span className="snap-option-state" aria-hidden="true">{snapOptions.neighbor ? '✓' : ''}</span></button><button role="menuitemcheckbox" aria-checked={snapOptions.node} className={`snap-option ${snapOptions.node ? 'is-enabled' : ''}`} onClick={() => onToggleSnapOption('node')}><span>节点吸附</span><span className="snap-option-state" aria-hidden="true">{snapOptions.node ? '✓' : ''}</span></button><button role="menuitemcheckbox" aria-checked={snapOptions.grid} className={`snap-option ${snapOptions.grid ? 'is-enabled' : ''}`} onClick={() => onToggleSnapOption('grid')}><span>网格吸附</span><span className="snap-option-state" aria-hidden="true">{snapOptions.grid ? '✓' : ''}</span></button></TopDropdown></div>
