...
export function LinePanel({project,selection,activeLineId,onSelect,onChange,onAddLine}:{project:ActualRouteProject;selection:Selection;activeLineId:string|null;onSelect:(id:string)=>void;onChange:(project:ActualRouteProject)=>void;onAddLine:()=>void}){
  ...
  return <aside className="left-panel panel" aria-label="线路结构">
    ...
    <div className="line-list">{project.lines.map(line=><button key={line.id} className={`line-row ${activeLineId===line.id||(selection?.type==='line'&&selection.id===line.id)?'selected':''}`} onClick={()=>onSelect(line.id)}><input aria-label={`${line.name}颜色`} className="line-color-input" type="color" value={line.color} onClick={event=>event.stopPropagation()} onChange={event=>patch(next=>{next.lines.find(item=>item.id===line.id)!.color=event.target.value})}/><span className="line-name">{line.name}</span><span className="station-count">{line.stationSequence.length}</span><input aria-label={`${line.name}显示`} title="显示" type="checkbox" checked={line.visible} onClick={event=>event.stopPropagation()} onChange={event=>patch(next=>{next.lines.find(item=>item.id===line.id)!.visible=event.target.checked})}/><input aria-label={`${line.name}锁定`} title="锁定" type="checkbox" checked={line.locked} onClick={event=>event.stopPropagation()} onChange={event=>patch(next=>{next.lines.find(item=>item.id===line.id)!.locked=event.target.checked})}/></button>)}</div>
    <section className="network-summary" aria-label="线网统计">
      <div className="network-total"><strong>全网</strong><span>{totalLength.toFixed(1)} km</span><span>{totalStations} 座车站</span></div>
      <div className="network-line-stats">{lineStats.map(({line,length,stations})=><div key={line.id}><i style={{background:line.color}}/><strong>{line.name}</strong><span>{length.toFixed(1)} km</span><span>{stations} 站</span></div>)}</div>
    </section>
