...
const openDate = (project: ActualRouteProject, segment: Segment) => {
  if (segment.openedAt) return segment.openedAt
  const baselineLineId = normalizeSegmentLineHistory(segment.lineHistory)?.find(entry => entry.effectiveAt === null)?.lineId ?? segment.lineId
  return project.lines.find(line => line.id === baselineLineId)?.openedAt || project.lines.find(line => line.id === segment.lineId)?.openedAt || ''
}
...
export function compileHistoryEvents(project: ActualRouteProject, settings: PresentationSettings = project.presentation): HistoryEvent[] {
  ...
  for (const segment of project.geometry.segments) {
    const history = normalizeSegmentLineHistory(segment.lineHistory)
    if (!history) continue
    for (let index = 0; index < history.length; index += 1) {
      const entry = history[index]
      if (entry.effectiveAt === null || !validDate(entry.effectiveAt) || entry.effectiveAt < start || entry.effectiveAt > end) continue
      const fromLineId = history[index - 1]?.lineId ?? segment.lineId
      if (!fromLineId || fromLineId === entry.lineId) continue
      const key = `${entry.effectiveAt}\u0000${fromLineId}\u0000${entry.lineId}`
      reassignmentGroups.set(key, [...(reassignmentGroups.get(key) ?? []), segment])
    }
  }
  for (const [key, segments] of reassignmentGroups) {
    const [historyDate, fromLineId, toLineId] = key.split('\u0000')
    for (const [componentIndex, component] of connectedComponents(segments).entries()) {
      events.push({ id: `${historyDate}-${fromLineId}-${toLineId}-reassignment-${componentIndex}`, type: 'LINE_REASSIGNMENT', eventTypes: ['LINE_REASSIGNMENT'], historyDate, lineId: toLineId, segmentIds: component.map(segment => segment.id), stationIds: [], interchangeStationIds: [], branches: [], lineReassignment: { fromLineId, toLineId } })
    }
  }
  const reassignedStationKeys = new Set<string>()
  for (const segment of project.geometry.segments) {
    const history = normalizeSegmentLineHistory(segment.lineHistory)
    if (!history) continue
    for (const entry of history) if (entry.effectiveAt !== null && validDate(entry.effectiveAt)) {
      for (const stationId of [segment.fromStationId, segment.toStationId]) if (activeLineIds(project, stationId, previousDate(entry.effectiveAt)).length > 0) reassignedStationKeys.add(`${entry.effectiveAt}\u0000${entry.lineId}\u0000${stationId}`)
    }
  }
  ...
  for (const relation of project.stationLineRelations) {
    ...
    if (!validDate(historyDate) || historyDate < start || historyDate > end || covered.has(`${historyDate}\u0000${relation.lineId}\u0000${relation.stationId}`) || reassignedStationKeys.has(`${historyDate}\u0000${relation.lineId}\u0000${relation.stationId}`)) continue
  ...
  for (const line of project.lines) {
    ...
    const colorHistory = normalizeLineColorHistory(line)
    if (colorHistory) for (const entry of colorHistory) {
      if (!validDate(entry.effectiveAt) || entry.effectiveAt < start || entry.effectiveAt > end) continue
...
function buildCompileCache(project: ActualRouteProject, beats: PresentationBeat[]): PresentationCompileCache {
  ...
  const segmentLineIdsByDate: Record<string, Record<string, string>> = {}
  for (const date of dates) segmentLineIdsByDate[date] = Object.fromEntries(project.geometry.segments.map(segment => [segment.id, resolveSegmentLineAt(segment, date)]))
...
function directComponent(project: ActualRouteProject, component: Segment[], earlier: Segment[], phase?: OpeningPhase, lineId?: string, historyDate?: string): DirectedSegment[][] {
  ...
  const historicalProject = historyDate ? { ...project, geometry: { ...project.geometry, segments: project.geometry.segments.map(segment => ({ ...segment, lineId: resolveSegmentLineAt(segment, historyDate) })) } } : project
  const historicalSegments = new Map(historicalProject.geometry.segments.map(segment => [segment.id, segment]))
  ...
  const line = project.lines.find(item => item.id === (lineId ?? component[0]?.lineId)), order = (id: string) => { const value = line?.stationSequence.indexOf(id) ?? -1; return value < 0 ? 1e9 : value }
  ...
  const walk = (seed: string) => { let stationId = seed; const branch: Omit<DirectedSegment, 'startRatio' | 'endRatio'>[] = []; while (true) { const candidates = (byStation.get(stationId) ?? []).filter(segment => unvisited.has(segment.id)).sort((a, b) => segmentOrder(historicalProject, historicalSegments.get(a.id) ?? a, lineId) - segmentOrder(historicalProject, historicalSegments.get(b.id) ?? b, lineId) || a.id.localeCompare(b.id)); const segment = candidates[0]; if (!segment) break; unvisited.delete(segment.id); const toStationId = segment.fromStationId === stationId ? segment.toStationId : segment.fromStationId; branch.push({ segmentId: segment.id, fromStationId: stationId, toStationId, length: getSegmentCurveLength(historicalProject, historicalSegments.get(segment.id) ?? segment) }); stationId = toStationId; if ((byStation.get(stationId) ?? []).filter(item => unvisited.has(item.id)).length > 1) break } if (branch.length) branches.push(withRatios(branch)); for (const candidate of (byStation.get(stationId) ?? []).filter(segment => unvisited.has(segment.id))) walk(stationId) }
...
function segmentOrder(project: ActualRouteProject, segment: Segment, ownerLineId?: string) { const line = project.lines.find(item => item.id === (ownerLineId ?? segment.lineId)), a = line?.stationSequence.indexOf(segment.fromStationId) ?? -1, b = line?.stationSequence.indexOf(segment.toStationId) ?? -1; return Math.min(a < 0 ? 1e9 : a, b < 0 ? 1e9 : b) }
